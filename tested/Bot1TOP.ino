/* RVTOP-Ccode 
 * Bot1 pairing: flash Bot1TOP only if bottom cannot be accessed.
 * Top sends legacy M+P+C packets; bottom on vehicle is unchanged.
...
#define DEPTH_HOLD_ENABLED 0       // Bot1 depth sensor broken — manual vertical only
...
#define DEPTH_FILTER_ALPHA 0.10f
#define DEPTH_UNITS_SCALE 1.0f
// ----------------- Motor configuration ------------------ //
...
float DriveGainL  = 0.58;
float DriveGainR  = 0.58;
float SteerGain   = -0.60;
float DiveGainL   = 0.55;
float DiveGainR   = 0.55;
float StrafeGain  = 0.58;
float GripperGain = 0.40;
#define GRIPPER_SLEW_STEP 0.025f
// Flip to -1.0f in pool if a stick axis runs backward (one constant at a time).
#define DEPTH_STICK_SIGN   (-1.0f)  // Left Y: up = surface
#define STRAFE_STICK_SIGN  (1.0f)   // Left X: right = strafe right
#define DRIVE_STICK_SIGN   (-1.0f)  // Right Y: up = forward (Xbox axis 5)
#define STEER_STICK_SIGN   (1.0f)   // Right X: right = turn right
...
void display_all_telems(void) {
  ...
  lcd.print(buf);
  lcd.print("    ");
  sprintf(buf, "Depth:  (sensor N/A)");
  lcd.setCursor(0, 2);
  lcd.print(buf);
  lcd.print("   ");
  sprintf(buf, "L:Depth/Strafe R:Drv/Turn");
  lcd.setCursor(0, 3);
  lcd.print(buf);
  lcd.print("  ");
}
...
/*
Purpose:
Reads D-pad direction from button bits, raw report flags, and POV hat axis.
Inputs:
buttons[], butts, and connected USB joysticks.
Outputs:
Four booleans for up/down/left/right press state.
Side Effects:
None.
Dependencies:
Called only while a gamepad frame is being processed.
Notes:
Unified across Bot1/Bot2 for reliable Xbox D-pad reporting variants.
DPadX: left/right -> servos[0] LED brightness; DPadY: up/down -> servos[1] camera tilt.
*/
...
/*
Purpose:
Updates synthesized D-pad axes used for LED dimming and camera tilt servos.
Inputs:
D-pad direction state and current analogs[DPadX]/analogs[DPadY].
Outputs:
Clamped -1..+1 values in analogs[DPadX] and analogs[DPadY].
Side Effects:
None on thruster axes.
Dependencies:
translate_controls_to_commands() maps DPadX -> servos[0], -DPadY -> servos[1].
Notes:
Face-button synthesized axes (ButsX/ButsY) are updated separately.
*/
...
void updateDepthHoldAssist(float dtSeconds) {
  ...
  // depth hold disabled on Bot1 — sensor not installed/working
#endif
}
...
void translate_response_to_controls() {
  ...
  // read the xbox controller over USB
    if (!joysticks[joystick_index].available()) continue;
...
/* calculate the motor outputs based on the controls and trims
  ...
  LEFT stick:
    Y = depth (up = surface, down = dive)
    X = strafe (left/right)
  RIGHT stick:
    Y = forward / reverse
    X = turn left / right
  Triggers = gripper
  D-pad = LED (L/R) and camera tilt (U/D)
*/

void translate_controls_to_commands() {
  float depthCmd  = analogs[LJoyY] * DEPTH_STICK_SIGN;
  float strafeCmd = analogs[LJoyX] * STRAFE_STICK_SIGN;
  float driveCmd  = analogs[RJoyY] * DRIVE_STICK_SIGN;
  float steerCmd  = analogs[RJoyX] * STEER_STICK_SIGN;

  float LRForeAft = driveCmd * DriveGainL + steerCmd * SteerGain;
  float RRForeAft = driveCmd * DriveGainR - steerCmd * SteerGain;
  float Strafe    = strafeCmd * StrafeGain;
  float LUpDown   = depthCmd * DiveGainL;
  float RUpDown   = depthCmd * DiveGainR;
...
/*
Purpose:
Builds the top-side RS-485 command packet from current motor and servo command arrays.
Inputs:
Global motors[] and servos[].
Outputs:
Writes a newline-terminated command packet into msg.
Side Effects:
None until Serial1.print() sends the buffer.
Dependencies:
Legacy Bot1Bottom expects M + P + C (no S section).
Notes:
Y/B switch state is kept locally in switches[] but is not transmitted on Bot1.
*/
