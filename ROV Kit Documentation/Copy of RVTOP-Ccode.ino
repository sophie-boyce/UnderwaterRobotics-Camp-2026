...
//    measurement         Battery   not    not  LEDtemp  H2Otemp  Depth
//    units                Volts    used   used   degC    degC    Feet
float telZeros[NVOLTS] = {   0.00,  0.40,  0.10,   1.40,   1.40,  0.50};
float telScale[NVOLTS] = {  11.00,  6.00,  8.00,  70.00,  71.00, 18.20};
...
void display_all_telems(void) {
  ...
  lcd.setCursor(0, 2);
  lcd.print(buf);
  sprintf(buf, "H2O temp %5.1f C", telems[4]);
  lcd.setCursor(0, 3);
  lcd.print(buf);
}
...
void translate_controls_to_commands() {
  ...
  float Gripper   = analogs[RTrig] - analogs[LTrig];
